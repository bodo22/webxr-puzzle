...
import { useThree, useFrame, Object3DNode, extend, createPortal } from '@react-three/fiber'
...
class FakeInputSourceFactory {
  ...
  createFakeInputSource(handedness) {
    return {
      ...
      profiles: ['oculus-hand', 'generic-hand', 'generic-hand-select', 'generic-trigger'],
...
export function RemoteControllers({ controllers }) {
  ...
  React.useLayoutEffect(() => {
    for (const target of controllers) {
      const fakeInputSource = fakeInputSourceFactory.createFakeInputSource(target.index ? 'right' : 'left')
      target.controller.dispatchEvent({ type: 'connected', data: fakeInputSource, fake: true })
    }
  }, [controllers])
  ...
  return (
    <>
      {controllers.map((target, i) => (
        <React.Fragment key={i}>{createPortal(<controllerModel args={[target]} />, target.grip)}</React.Fragment>
      ))}
    </>
  )
}
...
export function RemoteHands({ controllers, modelLeft, modelRight }) {
  ...
  React.useLayoutEffect(() => {
    for (const target of controllers) {
      const fakeInputSource = fakeInputSourceFactory.createFakeInputSource(target.index ? 'right' : 'left')
      target.hand.dispatchEvent({ type: 'connected', data: fakeInputSource, fake: true })
    }
  }, [controllers, modelLeft, modelRight])

  return (
    <>{controllers.map(({ hand }) => createPortal(<oculusHandModel args={[hand, modelLeft, modelRight]} />, hand))}</>
  )
}
...
export default function RemoteHandsAndController() {
  ...
  React.useEffect(() => {
    ...
    function handDataRecieved(data) {
      ...
      //   target.webXRController.update(fakeInputSource, fakeFrame, fakeReferenceSpace)
      // })
    }
